...
final goRouterProvider = Provider.autoDispose<GoRouter>((ref) {
  final goRouter = GoRouter(
    ...
    redirect: (context, state) {
      ...
      debugPrint('state.uri.path: ${state.uri.path} ${AppRoute.orders.path}');
      // user is logged in
      if (isLoggedIn) {
        if (state.uri.path == AppRoute.signIn.path) {
          return AppRoute.home.path;
        }
      } else {
        if (state.uri.path == AppRoute.orders.path ||
            state.uri.path == AppRoute.account.path) {
